...
int ng_audio_init (ngAudio* a) {
	...
	a->spec = obtained;
	a->device = device_id;
	a->data = malloc(4096 * sizeof(uint8_t));
	a->samples = 4096;
	a->playing = false;
...
int ng_clip_init (ngAudio* a, ngClip* clip, const char* file) {
	...
	uint8_t* buffer = malloc(buffer_max * sizeof(uint8_t));
	...
	clip->data = malloc(buffer_length * sizeof(uint8_t));
...
void ng_channel_start_sound (ngChannel* c, ngClip* clip, int mode) { // add sound
	...
	sound.sample = 0;
	sound.mode = mode;
	c->data = realloc(c->data, (c->sounds+1) * sizeof(ngSound));
